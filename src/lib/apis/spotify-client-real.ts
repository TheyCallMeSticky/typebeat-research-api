 ...
 */
export class SpotifyClientReal {
  ...
  /**
   * OBTENIR LES ARTISTES SIMILAIRES VIA RELATED-ARTISTS
   * HYPOTHÈSE: related-artists endpoint retourne jusqu'à 20 artistes similaires
   * FORMAT ATTENDU: { artists: SpotifyArtist[] }
   * INFÉRENCE: Artistes triés par similarité (algorithme Spotify)
   */
  async getRelatedArtists(artistId: string): Promise<SpotifyArtist[]> {
    try {
      ...
      console.log(`✅ Found ${response.data.artists.length} related artists`);
      return response.data.artists;
    } catch (error) {
      console.error(`❌ Failed to get related artists for ${artistId}:`, error);
      throw new Error(`Failed to get related artists: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
   ...
   */
  async getRecommendedArtists(seedArtistIds: string[], limit: number = 20): Promise<SpotifyArtist[]> {
    try {
      console.log(`🎵 Getting recommendations for ${seedArtistIds.length} seed artists`);
      
      const response = await this.client.get<SpotifyRecommendationsResponse>('/recommendations', {
        params: {
          seed_artists: seedArtistIds.slice(0, 5).join(','), // Max 5 seeds
  ...
  async findSimilarArtists(artistName: string, options: {
    ...
    try {
      ...
      const relatedArtists = await this.getRelatedArtists(mainArtist.id);
      ...
      relatedArtists.forEach(artist => allSimilarArtists.set(artist.id, artist));
...
export function createSpotifyClientReal(): SpotifyClientReal {
  return new SpotifyClientReal();
}
